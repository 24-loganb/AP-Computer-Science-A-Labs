	...
	public boolean processGuess(String guess)
	{
		...
		if (guess.equalsIgnoreCase(gameCelebrity.getAnswer()))
		{
			if (celebGameList.size() != 0)
			{
				...
				gameCelebrity = celebGameList.iterator().next();
				return true;
			}
			else
			{
			...
			//test to see how many items in list
			//if items in list, remove celebrity, set the next one on the list to gameCelebrity
			//if no items, set gameCelebrity to a new Celebrity with "" and "" for clue and name
		}
		else
			return false;
	}
