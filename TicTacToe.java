...
public class TicTacToe
{
    ...
    public TicTacToe()
    {
        ...
        double randomDouble = tester.nextDouble();
        if (randomDouble >= 0.5)
            player1Turn = true;
        else
            player1Turn = false;
    ...
    public String horizontalWinner()
    {
        for (int i = 0; i < board.length; i++)
        {
            boolean rowFullOfXs = true;
            boolean rowFullOfOs = true;
            for (int j = 0; j < board[i].length; j++)
            {
    ...
    public String verticalWinner()
    {
        ...
        {
            boolean columnFullOfXs = true;
            boolean columnFullOfOs = true;
            for (int i = 0; i < board.length; i++)
            {
                if (!board[i][j].equals(player1))
                    columnFullOfXs = false;
                if (!board[i][j].equals(player2))
                    columnFullOfOs = false;
            }
            if (columnFullOfXs)
            {
                win = true;
    ...
    {
        boolean leftDiagonalFullOfXs = true;
        boolean leftDiagonalFullOfOs = true;
        boolean rightDiagonalFullOfXs = true;
        boolean rightDiagonalFullOfOs = true;
        for (int i = 0; i < board.length; i++)
        {
            if (!board[i][i].equals(player1))
                leftDiagonalFullOfXs = false;
            if (!board[i][i].equals(player2))
                leftDiagonalFullOfOs = false;
            if (!board[i][board.length - 1 - i].equals(player1))
                rightDiagonalFullOfXs = false;
            if (!board[i][board.length - 1 - i].equals(player2))
                rightDiagonalFullOfOs = false;
        }
        if (leftDiagonalFullOfXs)
        {
            win = true;
            return player1;
        }
        if (leftDiagonalFullOfOs)
        {
            win = true;
            return player2;
        }
        if (rightDiagonalFullOfXs)
        {
            win = true;
            return player1;
        }
        if (rightDiagonalFullOfOs)
        {
            win = true;
            return player2;
        }
    ...
    {
        String winner = horizontalWinner();
        if (winner != null)
        {
            return winner + " wins!";
        }
